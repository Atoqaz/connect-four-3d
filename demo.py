...
class TicTacToe:
    def __init__(self, player="X", reward_type="goal_reward"):
        ...
        playerX_states = {
            state
            for state in states
            if (state.count(0) % 2 == 1 and state.count(1) == state.count(2))
        }  #
        playerO_states = {
            state
            for state in states
            if (state.count(0) % 2 == 0 and state.count(1) == (state.count(2) + 1))
        }
    ...
    def my_move(self, position):
        ...
        assert (
            self.win(self.me) == False and self.win(self.opponent) == False
...
def play_games(
    ...
    Q_X,
    Q_O,
    X_strategy="eps_greedy",
    O_strategy="eps_greedy",
    eps_X=0.05,
    eps_O=0.05,
    seed=1,
):
    ...
    for i in tqdm(range(n_games), position=0, leave=True):
        ...
        while True:
            ...
            if (
                t_board_X.is_game_over()
            ):  # need to end game here if X makes the winning move
                break
            # O plays second
            ...
            if (
                t_board_O.is_game_over()
            ):  # need to end game here if O makes the winnng move
                break
...
def get_win_statistics(
    ...
    return (
        (win_stats_df),
        (winning_sequences_X_list, winning_sequences_O_list),
        (X_first_actions_list, winning_X_first_actions_list),
        (O_first_actions_list, winning_O_first_actions_list),
    )
...
def get_win_rate(first_actions_list, winning_first_actions_list):
    ...
    win_rate = np.array(
        [
            winning_first_actions.get(key, 0) / first_actions.get(key, 1)
            for key in sorted(map_2d_1d.keys())
        ]
    ).reshape(3, 3)
...
def get_win_seqs(winning_sequences_list):
    ...
    win_seq_df = pd.DataFrame(
        {"winning_sequence": list(temp_dict.keys()), "N": list(temp_dict.values())}
    )
...
def plot_results(win_statistics):
    ...
    bplot = sns.boxplot(
        x="Result", y="N", data=win_stats_df_long, ax=axs[0, 0]
    ).set_title("Distribution of Wins,Losses and Ties")
    ...
    _ = sns.barplot(
        ...
        y="N",
        data=win_seq_X,
        order=["R1", "R2", "R3", "C1", "C2", "C3", "D1", "D2"],
        ax=axs[1, 1],
    ).set_title("Player X: winning sequences")
    ...
    _ = sns.barplot(
        ...
        order=["R1", "R2", "R3", "C1", "C2", "C3", "D1", "D2"],
        ax=axs[1, 2],
    ).set_title("Player O: winning sequences")
...
def train(
    ...
    **kwargs
    ...
    if "Q_X" in kwargs:
        ...
        assert (
            train_X == False
        ), "Train flag should be set to False if Q table is being provided"
        Q_X = kwargs["Q_X"]
    else:
        ...
        while not (t_board_X.is_game_over() or t_board_O.is_game_over()):
            ...
            if train_X:
                x_action_ = t_board_X.pick_best_action(
                    Q_X, action_type="greedy", eps=0.05
                )  # best action from S_new
                x_action_1d = t_board_X.b2_to_s1[x_action_]
                Q_X[S_X][x_action1d] += alpha * (
                    R_X + gamma * Q_X[S_X_new][x_action_1d] - Q_X[S_X][x_action1d]
                )
            ...
            if train_O:
                o_action_ = t_board_O.pick_best_action(
                    Q_O, action_type="greedy", eps=0.05
                )  # best action from S_new
                o_action_1d = t_board_O.b2_to_s1[o_action_]
                Q_O[S_O][o_action1d] += alpha * (
                    R_O + gamma * Q_O[S_O_new][o_action_1d] - Q_O[S_O][o_action1d]
                )
...
win_statistics = get_win_statistics(
    Q_X,
    Q_O,
    sets=10,
    games_in_set=100,
